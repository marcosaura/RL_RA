...
class RingAttractorLayer(nn.Module):
    def __init__(self, input_size, output_size, num_neurons, connectivity_strength):
        ...
        self.rnn.weight_ih_l0 = nn.Parameter(torch.Tensor(self.append_row_and_columns(input_to_hidden, int(input_size/output_size), 0.1)))
...
class PredictionNetwork(nn.Module):
    def __init__(
        ...
        self.fc_weighting = nn.Linear(9 * 9, 1)  # Fully connected layer with input size 9*9 and output size 64
        self.fc_weighting_norm = nn.Softmax(dim=1)
        self.scaling_parameter =  nn.Parameter(torch.Tensor([1.0]))
        self.scaling_parameter_RA =  nn.Parameter(torch.Tensor([0.01]))


        RA_action_space = int(action_space_size/2)

        self.RA_layer = nn.Sequential(RingAttractorLayer(RA_action_space*8,RA_action_space,RA_action_space,0.1))
        self.RA_layer_2 = nn.Sequential(RingAttractorLayer(RA_action_space*8,RA_action_space,RA_action_space,0.1))
    ...
    def forward(self, x):
        ...
        policy1, policy2 = torch.split(policy, policy.size(1) // 2, dim=1)

        policy1 = self.RA_layer(policy1)
        policy2 = self.RA_layer_2(policy2)

        policy_mapping = torch.matmul(policy1, policy2)
        policy_mapping = torch.flatten(policy_mapping)
        policy_weight = self.fc_weighting(policy_mapping)
        policy_weight = self.fc_weighting_norm(policy_weight)

        policy1 = torch.mul(policy1, 1/(1+policy_weight))
        policy2 = torch.mul(policy2, 1/(1-policy_weight))

        policy = torch.cat(policy1, policy2)
