...
class Options:
    def __init__(self):
        ...
        self.Target_update = 5000 # Update the target network each 10000 steps
...
E_W_0 = nd.normal(loc=0, scale=.01, shape=(num_action,opt.lastlayer),ctx = opt.ctx)
E_W_1 = nd.normal(loc=0, scale=.01, shape=(num_action,opt.lastlayer),ctx = opt.ctx)
E_W_2 = nd.normal(loc=0, scale=.01, shape=(num_action,opt.lastlayer),ctx = opt.ctx)
E_W_3 = nd.normal(loc=0, scale=.01, shape=(num_action,opt.lastlayer),ctx = opt.ctx)
E_W_4 = nd.normal(loc=0, scale=.01, shape=(num_action,opt.lastlayer),ctx = opt.ctx)
E_W_5 = nd.normal(loc=0, scale=.01, shape=(num_action,opt.lastlayer),ctx = opt.ctx)
E_W_6 = nd.normal(loc=0, scale=.01, shape=(num_action,opt.lastlayer),ctx = opt.ctx)
E_W_7 = nd.normal(loc=0, scale=.01, shape=(num_action,opt.lastlayer),ctx = opt.ctx)
E_W_8 = nd.normal(loc=0, scale=.01, shape=(num_action,opt.lastlayer),ctx = opt.ctx)
E_W_9 = nd.normal(loc=0, scale=.01, shape=(num_action,opt.lastlayer),ctx = opt.ctx)
...
Cov_W_0 = nd.normal(loc=0, scale= 1, shape=(num_action,opt.lastlayer,opt.lastlayer),ctx = opt.ctx)+eye
Cov_W_1 = nd.normal(loc=0, scale= 1, shape=(num_action,opt.lastlayer,opt.lastlayer),ctx = opt.ctx)+eye
Cov_W_2 = nd.normal(loc=0, scale= 1, shape=(num_action,opt.lastlayer,opt.lastlayer),ctx = opt.ctx)+eye
Cov_W_3 = nd.normal(loc=0, scale= 1, shape=(num_action,opt.lastlayer,opt.lastlayer),ctx = opt.ctx)+eye
Cov_W_4 = nd.normal(loc=0, scale= 1, shape=(num_action,opt.lastlayer,opt.lastlayer),ctx = opt.ctx)+eye
Cov_W_5 = nd.normal(loc=0, scale= 1, shape=(num_action,opt.lastlayer,opt.lastlayer),ctx = opt.ctx)+eye
Cov_W_6 = nd.normal(loc=0, scale= 1, shape=(num_action,opt.lastlayer,opt.lastlayer),ctx = opt.ctx)+eye
Cov_W_7 = nd.normal(loc=0, scale= 1, shape=(num_action,opt.lastlayer,opt.lastlayer),ctx = opt.ctx)+eye
Cov_W_8 = nd.normal(loc=0, scale= 1, shape=(num_action,opt.lastlayer,opt.lastlayer),ctx = opt.ctx)+eye
Cov_W_9 = nd.normal(loc=0, scale= 1, shape=(num_action,opt.lastlayer,opt.lastlayer),ctx = opt.ctx)+eye

Cov_W_decom_0 = Cov_W_0
Cov_W_decom_1 = Cov_W_1
Cov_W_decom_2 = Cov_W_2
Cov_W_decom_3 = Cov_W_3
Cov_W_decom_4 = Cov_W_4
Cov_W_decom_5 = Cov_W_5
Cov_W_decom_6 = Cov_W_6
Cov_W_decom_7 = Cov_W_7
Cov_W_decom_8 = Cov_W_8
Cov_W_decom_9 = Cov_W_9

for i in range(num_action):
    Cov_W_0[i] = eye
    Cov_W_decom_0[i] = nd.array(np.linalg.cholesky(((Cov_W_0[i]+nd.transpose(Cov_W_0[i]))/2.).asnumpy()),ctx = opt.ctx)

    Cov_W_1[i] = eye
    Cov_W_decom_1[i] = nd.array(np.linalg.cholesky(((Cov_W_1[i]+nd.transpose(Cov_W_1[i]))/2.).asnumpy()),ctx = opt.ctx)

    Cov_W_2[i] = eye
    Cov_W_decom_2[i] = nd.array(np.linalg.cholesky(((Cov_W_2[i]+nd.transpose(Cov_W_2[i]))/2.).asnumpy()),ctx = opt.ctx)

    Cov_W_3[i] = eye
    Cov_W_decom_3[i] = nd.array(np.linalg.cholesky(((Cov_W_3[i]+nd.transpose(Cov_W_3[i]))/2.).asnumpy()),ctx = opt.ctx)

    Cov_W_4[i] = eye
    Cov_W_decom_4[i] = nd.array(np.linalg.cholesky(((Cov_W_4[i]+nd.transpose(Cov_W_4[i]))/2.).asnumpy()),ctx = opt.ctx)

    Cov_W_5[i] = eye
    Cov_W_decom_5[i] = nd.array(np.linalg.cholesky(((Cov_W_5[i]+nd.transpose(Cov_W_5[i]))/2.).asnumpy()),ctx = opt.ctx)

    Cov_W_6[i] = eye
    Cov_W_decom_6[i] = nd.array(np.linalg.cholesky(((Cov_W_6[i]+nd.transpose(Cov_W_6[i]))/2.).asnumpy()),ctx = opt.ctx)

    Cov_W_7[i] = eye
    Cov_W_decom_7[i] = nd.array(np.linalg.cholesky(((Cov_W_7[i]+nd.transpose(Cov_W_7[i]))/2.).asnumpy()),ctx = opt.ctx)

    Cov_W_8[i] = eye
    Cov_W_decom_8[i] = nd.array(np.linalg.cholesky(((Cov_W_8[i]+nd.transpose(Cov_W_8[i]))/2.).asnumpy()),ctx = opt.ctx)

    Cov_W_9[i] = eye
    Cov_W_decom_9[i] = nd.array(np.linalg.cholesky(((Cov_W_9[i]+nd.transpose(Cov_W_9[i]))/2.).asnumpy()),ctx = opt.ctx)


Cov_W_target = (Cov_W_0 + Cov_W_1 + Cov_W_2 + Cov_W_3 + Cov_W_4 + Cov_W_5 + Cov_W_6 + Cov_W_7 + Cov_W_8 + Cov_W_9)/10


phiphiT_0 = nd.zeros((num_action,opt.lastlayer,opt.lastlayer), opt.ctx)
phiY_0 = nd.zeros((num_action,opt.lastlayer), opt.ctx)

phiphiT_1 = nd.zeros((num_action,opt.lastlayer,opt.lastlayer), opt.ctx)
phiY_1 = nd.zeros((num_action,opt.lastlayer), opt.ctx)

phiphiT_2 = nd.zeros((num_action,opt.lastlayer,opt.lastlayer), opt.ctx)
phiY_2 = nd.zeros((num_action,opt.lastlayer), opt.ctx)

phiphiT_2 = nd.zeros((num_action,opt.lastlayer,opt.lastlayer), opt.ctx)
phiY_2 = nd.zeros((num_action,opt.lastlayer), opt.ctx)

phiphiT_3 = nd.zeros((num_action,opt.lastlayer,opt.lastlayer), opt.ctx)
phiY_3 = nd.zeros((num_action,opt.lastlayer), opt.ctx)

phiphiT_4 = nd.zeros((num_action,opt.lastlayer,opt.lastlayer), opt.ctx)
phiY_4 = nd.zeros((num_action,opt.lastlayer), opt.ctx)

phiphiT_5 = nd.zeros((num_action,opt.lastlayer,opt.lastlayer), opt.ctx)
phiY_5 = nd.zeros((num_action,opt.lastlayer), opt.ctx)

phiphiT_6 = nd.zeros((num_action,opt.lastlayer,opt.lastlayer), opt.ctx)
phiY_6 = nd.zeros((num_action,opt.lastlayer), opt.ctx)

phiphiT_7 = nd.zeros((num_action,opt.lastlayer,opt.lastlayer), opt.ctx)
phiY_7 = nd.zeros((num_action,opt.lastlayer), opt.ctx)

phiphiT_8 = nd.zeros((num_action,opt.lastlayer,opt.lastlayer), opt.ctx)
phiY_8 = nd.zeros((num_action,opt.lastlayer), opt.ctx)

phiphiT_9 = nd.zeros((num_action,opt.lastlayer,opt.lastlayer), opt.ctx)
phiY_9 = nd.zeros((num_action,opt.lastlayer), opt.ctx)

sigma = opt.sigma
sigma_n = opt.sigma_n
new_episode_write = False

def BayesReg(phiphiT,phiY,alpha,batch_size, E_W_x, Cov_W_x):
    ...
    for i in range(num_action):
        ...
        E_W_x[i] = nd.array(np.dot(inv,phiY[i].asnumpy())/opt.sigma_n, ctx = opt.ctx)
        Cov_W_x[i] = opt.sigma * inv
    return phiphiT,phiY,E_W_x,Cov_W_x 
...
while epis_count < opt.max_episode:
    ...
    while not done or not info["flag_get"]:
        ...
        if frame_counter % opt.f_sampling:
            ...
            E_W_M[0] = sample_W(E_W_0,E_W_M[0],Cov_W_decom_0)
            E_W_M[1] = sample_W(E_W_1,E_W_M[1],Cov_W_decom_1)
            E_W_M[2] = sample_W(E_W_2,E_W_M[2],Cov_W_decom_2)
            E_W_M[3] = sample_W(E_W_3,E_W_M[3],Cov_W_decom_3)
            E_W_M[4] = sample_W(E_W_4,E_W_M[4],Cov_W_decom_4)
            E_W_M[5] = sample_W(E_W_5,E_W_M[5],Cov_W_decom_5)
            E_W_M[6] = sample_W(E_W_6,E_W_M[6],Cov_W_decom_6)
            E_W_M[7] = sample_W(E_W_7,E_W_M[7],Cov_W_decom_7)
            E_W_M[8] = sample_W(E_W_8,E_W_M[8],Cov_W_decom_8)
            E_W_M[9] = sample_W(E_W_9,E_W_M[9],Cov_W_decom_9)
        ...
        if frame_counter > opt.replay_start_size:
            if frame_counter % opt.Target_update == 0 :
                ...
                if c_t == opt.target_W_update:
                    phiphiT_0,phiY_0,E_W_0,Cov_W_0 = BayesReg(phiphiT_0,phiY_0,opt.alpha_target,opt.BayesBatch, E_W_0, Cov_W_0)
                    phiphiT_1,phiY_1,E_W_1,Cov_W_1 = BayesReg(phiphiT_1,phiY_1,opt.alpha_target,opt.BayesBatch, E_W_1, Cov_W_1)
                    phiphiT_2,phiY_2,E_W_2,Cov_W_2 = BayesReg(phiphiT_2,phiY_2,opt.alpha_target,opt.BayesBatch, E_W_2, Cov_W_2)
                    phiphiT_3,phiY_3,E_W_3,Cov_W_3 = BayesReg(phiphiT_3,phiY_3,opt.alpha_target,opt.BayesBatch, E_W_3, Cov_W_3)
                    phiphiT_4,phiY_4,E_W_4,Cov_W_4 = BayesReg(phiphiT_4,phiY_4,opt.alpha_target,opt.BayesBatch, E_W_4, Cov_W_4)
                    phiphiT_5,phiY_5,E_W_5,Cov_W_5 = BayesReg(phiphiT_5,phiY_5,opt.alpha_target,opt.BayesBatch, E_W_5, Cov_W_5)
                    phiphiT_6,phiY_6,E_W_6,Cov_W_6 = BayesReg(phiphiT_6,phiY_6,opt.alpha_target,opt.BayesBatch, E_W_6, Cov_W_6)
                    phiphiT_7,phiY_7,E_W_7,Cov_W_7 = BayesReg(phiphiT_7,phiY_7,opt.alpha_target,opt.BayesBatch, E_W_7, Cov_W_7)
                    phiphiT_8,phiY_8,E_W_8,Cov_W_8 = BayesReg(phiphiT_8,phiY_8,opt.alpha_target,opt.BayesBatch, E_W_8, Cov_W_8)
                    phiphiT_9,phiY_9,E_W_9,Cov_W_9 = BayesReg(phiphiT_9,phiY_9,opt.alpha_target,opt.BayesBatch, E_W_9, Cov_W_9)


                    E_W_target = (E_W_0 + E_W_1 + E_W_2 + E_W_3 + E_W_4 + E_W_5 + E_W_6 + E_W_7 + E_W_8 + E_W_9)/10
                    Cov_W_target = (Cov_W_0 + Cov_W_1 + Cov_W_2 + Cov_W_3 + Cov_W_4 + Cov_W_5 + Cov_W_6 + Cov_W_7 + Cov_W_8 + Cov_W_9)/10
                    ...
                    for ii in range(num_action):
                        Cov_W_decom_0[ii] = nd.array(np.linalg.cholesky(((Cov_W_0[ii]+nd.transpose(Cov_W_0[ii]))/2.).asnumpy()),ctx = opt.ctx)
                        Cov_W_decom_1[ii] = nd.array(np.linalg.cholesky(((Cov_W_1[ii]+nd.transpose(Cov_W_1[ii]))/2.).asnumpy()),ctx = opt.ctx)
                        Cov_W_decom_2[ii] = nd.array(np.linalg.cholesky(((Cov_W_2[ii]+nd.transpose(Cov_W_2[ii]))/2.).asnumpy()),ctx = opt.ctx)
                        Cov_W_decom_3[ii] = nd.array(np.linalg.cholesky(((Cov_W_3[ii]+nd.transpose(Cov_W_3[ii]))/2.).asnumpy()),ctx = opt.ctx)
                        Cov_W_decom_4[ii] = nd.array(np.linalg.cholesky(((Cov_W_4[ii]+nd.transpose(Cov_W_4[ii]))/2.).asnumpy()),ctx = opt.ctx)
                        Cov_W_decom_5[ii] = nd.array(np.linalg.cholesky(((Cov_W_5[ii]+nd.transpose(Cov_W_5[ii]))/2.).asnumpy()),ctx = opt.ctx)
                        Cov_W_decom_6[ii] = nd.array(np.linalg.cholesky(((Cov_W_6[ii]+nd.transpose(Cov_W_6[ii]))/2.).asnumpy()),ctx = opt.ctx)
                        Cov_W_decom_7[ii] = nd.array(np.linalg.cholesky(((Cov_W_7[ii]+nd.transpose(Cov_W_7[ii]))/2.).asnumpy()),ctx = opt.ctx)
                        Cov_W_decom_8[ii] = nd.array(np.linalg.cholesky(((Cov_W_8[ii]+nd.transpose(Cov_W_8[ii]))/2.).asnumpy()),ctx = opt.ctx)
                        Cov_W_decom_9[ii] = nd.array(np.linalg.cholesky(((Cov_W_9[ii]+nd.transpose(Cov_W_9[ii]))/2.).asnumpy()),ctx = opt.ctx)
