...
from RA_heavy import RingAttractor
...
class DQN_node(Node):
    ...
    def __init__(self):
        ...
        super().__init__('BDQN',allow_undeclared_parameters=True, automatically_declare_parameters_from_overrides=True)

        self.get_logger().info(str("BDQN NODE STARTED" ))
    ...
    def init_publishers(self):
        ...
        self.DQN_time_publisher = self.create_publisher(Float32, "/TUoM/BDQN/time",  10)
        self.DQN_episode_publisher = self.create_publisher(Float32, "/TUoM/BDQN/episode",  10)
        self.DQN_frame_publisher = self.create_publisher(Float32, "/TUoM/BDQN/frame_number",  10)

        self.DQN_cm_reward_publisher = self.create_publisher(Float32, "/TUoM/BDQN/cummulative_reward",  10)
        self.DQN_cm_cl_reward_publisher = self.create_publisher(Float32, "/TUoM/BDQN/cummulative_clipped_reward",  10)
        self.DQN_tot_reward_publisher = self.create_publisher(Float32, "/TUoM/BDQN/total_reward",  10)
        self.DQN_tot_cl_reward_publisher = self.create_publisher(Float32, "/TUoM/BDQN/total_clipped_reward",  10)
        ...
        for action_pub in range (8):
            self.DQN_action_rew_publisher = self.create_publisher(Float32, "/TUoM/BDQN/action_reward_" + str(action_pub),  10)
        ...
        self.DQN_action_RA_publisher = self.create_publisher(Float32, "/TUoM/BDQN_RA_heavy/RA_action",  10)
        self.DQN_orientation_RA_publisher = self.create_publisher(Float32, "/TUoM/BDQN_RA_heavy/RA_orientation",  10)


    def publish_actions(self, action, reward):
...
while epis_count < opt.max_episode:
    ...
    while not done:
        ...
        action_array = a.asnumpy()

        # for action_list_n in range(len(action_array)):
        #     dqn_node.publish_actions(action_list_n, action_array[action_list_n] )
        ...
        if frame_counter % opt.f_sampling:
            ...
            if new_episode is True:
                # np.savetxt("/home/ds/Desktop/RA/BLR_W/MEW_episode_"+str(epis_count), E_W_.asnumpy(), delimiter=",")
                # np.savetxt("/home/ds/Desktop/RA/BLR_W/EW_episode_"+str(epis_count), E_W.asnumpy(), delimiter=",")
                # np.savetxt("/home/ds/Desktop/RA/BLR_W/Cov_episode_"+str(epis_count),  Cov_W_decom.asnumpy().reshape(Cov_W_decom.asnumpy().shape[0], -1), delimiter=",")
                new_episode = False
