...
def map_action(action_movements, action_Avalon):
...
dqn_node = DQN_node("BDQN_Avalon_RA")
...
while epis_count < opt.max_episode:
    ...
    while not done:
        ...
        action_values = nd.dot(E_W_,dqn_(data)[0].T)
        # action = np.argmax(a.asnumpy()).astype(np.uint8)

        actions_array[0][0] = action_values[0].asnumpy()[0]
        actions_array[0][1] = 0
        actions_array[0][2] = 60
    
        actions_array[1][0] = action_values[1].asnumpy()[0]
        actions_array[1][1] = 45
        actions_array[1][2] = 60
    
        actions_array[2][0] = action_values[2].asnumpy()[0]
        actions_array[2][1] = 90
        actions_array[2][2] = 60
    
        actions_array[3][0] = action_values[3].asnumpy()[0]
        actions_array[3][1] = 135
        actions_array[3][2] = 60

        actions_array[4][0] = action_values[4].asnumpy()[0]
        actions_array[4][1] = 180
        actions_array[4][2] = 60

        actions_array[5][0] = action_values[5].asnumpy()[0]
        actions_array[5][1] = 225
        actions_array[5][2] = 60

        actions_array[6][0] = action_values[6].asnumpy()[0]
        actions_array[6][1] = 270
        actions_array[6][2] = 60

        actions_array[7][0] = action_values[7].asnumpy()[0]
        actions_array[7][1] = 315
        actions_array[7][2] = 60


        dqn_node.publish_actions(0, actions_array[0][0], actions_array[0][2])
        dqn_node.publish_actions(1, actions_array[1][0], actions_array[1][2])
        dqn_node.publish_actions(2, actions_array[2][0], actions_array[2][2])
        dqn_node.publish_actions(3, actions_array[3][0], actions_array[3][2])
        dqn_node.publish_actions(4, actions_array[4][0], actions_array[4][2])
        dqn_node.publish_actions(5, actions_array[5][0], actions_array[5][2])
        dqn_node.publish_actions(6, actions_array[6][0], actions_array[6][2])
        dqn_node.publish_actions(7, actions_array[7][0], actions_array[7][2])

        actions_array[:,0] = 60*((actions_array[:,0]))/(np.max(actions_array[:,0]))
        ...
        action_cont = [np.cos(action_signal * np.pi/180), np.sin(action_signal * np.pi/180), -0.5*np.sin(action_signal * np.pi/180)]

        if action_signal > 336 or action_signal < 23: action = 0
        if action_signal > 22 and action_signal < 67: action = 1
        if action_signal > 66 and action_signal < 112: action = 2
        if action_signal > 111 and action_signal < 157: action = 3
        if action_signal > 157 and action_signal < 203: action = 4
        if action_signal > 202 and action_signal < 247: action = 5
        if action_signal > 246 and action_signal < 293: action = 6
        if action_signal > 292 and action_signal < 337: action = 7
        
        dqn_node.publish_actions_selection(int(action), action_signal)
        ...
        for skip in range(opt.skip_frame-1):
            next_frame, reward, done, info = env.step(map_action(action_cont, action_Avalon))
            next_frame = next_frame["rgbd"]
            renderimage(next_frame)
            cum_clipped_reward += rew_clipper(reward)
            rew += reward
            # for internal_skip in range(opt.internal_skip_frame-1):
            #     _ , reward, done, info = env.step(map_action(action_cont, action_Avalon))
            #     cum_clipped_reward += rew_clipper(reward)
            #     rew += reward
        next_frame_new, reward,  done, info = env.step(map_action(action_cont, action_Avalon))
