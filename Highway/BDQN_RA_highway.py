...
class Options:
    def __init__(self):
      ...
      #Articheture
        ...
        self.skip_frame = 0 # Skip 4-1 raw frames between steps
        self.internal_skip_frame = 0 # Skip 4-1 raw frames between skipped frames
        self.frame_len = 4 # Each state is formed as a concatination 4 step frames [f(t-12),f(t-8),f(t-4),f(t)]
        self.Target_update = 1000 # Update the target network each 10000 steps
        self.epsilon_min = 0.1 # Minimum level of stochasticity of policy (epsilon)-greedy
        self.annealing_end = 100. # The number of step it take to linearly anneal the epsilon to it min value
        self.gamma = 0.99 # The discount factor
        self.replay_start_size = 200 # Start to backpropagated through the network, learning starts
        ...
        self.target_batch_size = 1000 #target update sample batch
        self.BayesBatch = 1000 #size of batch for udpating E_W and Cov
...
num_action = 8# env.action_space.n # Extract the number of available action from the environment setting
...
class DQN_node(Node):
    ...
    def __init__(self, model_name):
        ...
        self.init_publishers(model_name)


    def init_publishers(self, model_name):
    
        self.DQN_time_publisher = self.create_publisher(Float32, "/TUoM/"+ model_name +"/time",  10)
        self.DQN_episode_publisher = self.create_publisher(Float32, "/TUoM/"+ model_name +"/episode",  10)
        self.DQN_frame_publisher = self.create_publisher(Float32, "/TUoM/"+ model_name +"/frame_number",  10)

        self.DQN_cm_reward_publisher = self.create_publisher(Float32, "/TUoM/"+ model_name +"/cummulative_reward",  10)
        self.DQN_cm_cl_reward_publisher = self.create_publisher(Float32, "/TUoM/"+ model_name +"/cummulative_clipped_reward",  10)
        self.DQN_tot_reward_publisher = self.create_publisher(Float32, "/TUoM/"+ model_name +"/total_reward",  10)
        self.DQN_tot_cl_reward_publisher = self.create_publisher(Float32, "/TUoM/"+ model_name +"/total_clipped_reward",  10)
        ...
        for action_pub in range (8):
            self.DQN_action_rew_publisher = self.create_publisher(Float32, "/TUoM/"+ model_name +"/action_reward_" + str(action_pub),  10)
            self.DQN_action_sigma_publisher = self.create_publisher(Float32, "/TUoM/"+ model_name +"/action_sigma_" + str(action_pub),  10)
        ...
        for action_pub in range (8):
            self.DQN_action_rew_publisher = self.create_publisher(Float32, "/TUoM/"+ model_name +"/norm_action_reward_" + str(action_pub),  10)
            self.DQN_action_sigma_publisher = self.create_publisher(Float32, "/TUoM/"+ model_name +"/norm_action_sigma_" + str(action_pub),  10)
        ...
        self.DQN_action_RA_publisher = self.create_publisher(Float32, "/TUoM/"+ model_name +"/RA_action",  10)
        self.DQN_orientation_RA_publisher = self.create_publisher(Float32, "/TUoM/"+ model_name +"/RA_orientation",  10)

        self.DQN_orientation_RA_publisher = self.create_publisher(Float32, "/TUoM/"+ model_name +"/loss",  10)
    ...
    def publish_loss(self, loss):

        self.loss_msg.data = float(loss)

        self.DQN_orientation_RA_publisher.publish(self.loss_msg)
...
dqn_node = DQN_node("BDQN_RA_4_choices")

print("SELECT 25 BDQN 75 RA ")
...
while epis_count < opt.max_episode:
    ...
    while not done:
        ...
        action_dqn = np.argmax(actions_BRL_matrix[0].asnumpy()).astype(np.uint8)
        miu_max = 0.000000001
        ...
        action = random.choice([action_dqn, action, action, action])
        # env.render()
        ...
        for skip in range(opt.skip_frame-1):
            next_frame, reward, done, truncate, info = env.step(map_action(action))
            if info["rewards"]["on_road_reward"]==0.0: done=True
            if info["speed"]<=0.0: done=True
            if truncate==True: done=True
            # renderimage(next_frame)
        ...
        if not done:
            next_frame_new, reward, done, truncate, info = env.step(map_action(action))
            if info["rewards"]["on_road_reward"]==0.0: done=True
            if info["speed"]<=0.0: done=True
            if truncate==True: done=True
            # renderimage(next_frame)
        ...
        if frame_counter > opt.replay_start_size: 
            if frame_counter % opt.learning_frequency == 0:
                ...
                with autograd.record():
                    ...
                    dqn_node.publish_loss(loss.asnumpy()[0].astype(np.float32))
                loss.backward()
    ...
    try: dqn_node.publish_results(epis_count,t+1,frame_counter,cum_clipped_reward,cum_reward,moving_average_clipped,moving_average)
    except: print("value type not compatible")
